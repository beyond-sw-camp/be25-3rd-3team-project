  ...
  <div class="min-h-screen bg-white">
    <header class="flex h-[72px] items-center justify-between px-6 min-[641px]:px-[40px] min-[981px]:px-[60px]">
      <div class="flex h-[40px] items-center gap-2">
        <div class="flex h-[40px] w-[40px] items-center justify-center rounded-[8px] bg-[#ff5a00]">
          <span class="text-[20px] font-bold text-white">A</span>
        </div>
        <span class="text-[20px] font-bold text-[#111827]">AutoSource</span>
      </div>

      <nav class="hidden h-[40px] items-center gap-8 min-[768px]:flex">
        <button class="flex h-[40px] items-center bg-transparent text-[15px] font-bold text-[#4b5563] transition-colors hover:text-[#111827]">사용 가이드</button>
        <button class="flex h-[40px] items-center bg-transparent text-[15px] font-bold text-[#4b5563] transition-colors hover:text-[#111827]">서비스 소개</button>
        <button class="flex h-[40px] items-center bg-transparent text-[15px] font-bold text-[#4b5563] transition-colors hover:text-[#111827]">가격 안내</button>
        <button class="flex h-[40px] items-center bg-transparent text-[15px] font-bold text-[#4b5563] transition-colors hover:text-[#111827]">고객센터</button>
      </nav>

      <div class="flex h-[40px] items-center gap-3">
        <template v-if="!isLoggedIn">
          <button class="flex h-[40px] items-center rounded-md bg-[#ff7b39] px-4 text-[14px] font-bold text-white transition-colors hover:bg-[#ee864b] min-[768px]:px-5" @click="goToLogin">
            로그인
          </button>
        </template>
        <template v-else>
          <button class="flex h-[40px] items-center rounded-md bg-[#ff7b39] px-5 text-[14px] font-bold text-white transition-colors hover:bg-[#ee864b]" @click="goToProfile">
            마이페이지
          </button>
          <button class="flex h-[40px] items-center rounded-md border border-[#d1d5db] bg-white px-5 text-[14px] font-bold text-[#374151] transition-colors hover:bg-[#f9fafb]" @click="handleLogout">
            로그아웃
          </button>
        </template>
      </div>
    ...
    <main class="relative flex min-h-[calc(100vh-72px)] flex-col items-center justify-center overflow-hidden bg-[#f8f8f8] px-6 pb-20 pt-10 min-[641px]:px-[40px] min-[981px]:px-[60px]">
      ...
      <div class="pointer-events-none absolute bottom-[20%] left-[10%] h-[120px] w-[120px] rotate-[-15deg] rounded-[20px] border-4 border-[#ffbf9e] opacity-[0.25]"></div>
      ...
      <div class="relative z-[2] w-full max-w-[800px] text-center">
        <div class="mb-10 mt-8 min-[641px]:mt-12">
          <h1 class="m-0 text-[48px] font-semibold leading-none text-[#ff5a00] min-[641px]:text-[68px] min-[981px]:text-[88px]">
            AutoSource
          </h1>
          <p class="mt-3 text-[16px] font-medium text-[#ff7b39] min-[641px]:text-[18px]">Sourcing Automation System</p>
        </div>

        <h2 class="m-0 text-[22px] font-bold leading-[1.4] text-[#111827] min-[641px]:text-[28px] min-[981px]:text-[32px]">
          자동 소싱으로 시작하는<br />가장 쉬운 구매 대행
        </h2>

        <p class="mb-10 mt-5 text-[16px] font-medium text-[#6b7280] min-[641px]:text-[18px]">
          AutoSource로 1분 만에 시작해보세요
        </p>
